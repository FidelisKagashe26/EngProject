...
export const PaymentsPage = () => {
  ...
  // Optional: apply the payment to one of the project's open (unpaid) invoices.
  const [invoiceId, setInvoiceId] = useState("");
  const [projectInvoices, setProjectInvoices] = useState<InvoiceApiRecord[]>([]);
  ...
  // While adding a payment, offer the project's open (unpaid) invoices to apply
  // it to; default to the newest one (usually the current auto-draft). Linking
  // marks that invoice as settled without touching amount_received twice.
  useEffect(() => {
    if (!showAddModal || editingPaymentId || !projectId) {
      setProjectInvoices([]);
      return;
    }
    let active = true;
    api
      ...
      .then((res) => {
        if (!active) return;
        const open = res.rows.filter((r) => r.balance > 0 && r.status !== "Cancelled");
        setProjectInvoices(open);
        setInvoiceId((cur) => (open.some((r) => r.id === cur) ? cur : open[0]?.id ?? ""));
      })
      .catch(() => {
        if (active) setProjectInvoices([]);
      });
    return () => {
      active = false;
    };
  }, [showAddModal, editingPaymentId, projectId]);
  ...
  const handleSavePayment = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        // Only send an invoice link when creating; empty means a standalone payment.
        invoiceId: editingPaymentId ? undefined : invoiceId || undefined,
    ...
    <div className="space-y-6">
      ...
      <DetailModal
        ...
        rows={viewPayment ? [
          ...
          { label: "Applied to Invoice", value: viewPayment.invoiceNumber || "-" },
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <SurfaceCard className="w-full max-w-2xl max-h-[90vh] overflow-y-auto" title={editingPaymentId ? "Edit Payment" : "Add Payment"}>
            <form className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              ...
              {!editingPaymentId && projectInvoices.length > 0 ? (
                <label className="form-field">
                  <span>Apply to Invoice (Optional)</span>
                  <GuiSelect className="input-field" onChange={(e) => setInvoiceId(e.target.value)} value={invoiceId}>
                    <option value="">— Create a new receipt invoice —</option>
                    {projectInvoices.map((inv) => (
                      <option key={inv.id} value={inv.id}>
                        {inv.number} · balance {formatTzs(inv.balance)}{inv.autoGenerated ? " · Auto" : ""}
                      </option>
                    ))}
                  </GuiSelect>
                  <span className="mt-1 text-[10px] text-slate-400">
                    Leave as “Create a new receipt invoice” and one is generated automatically for this payment.
                  </span>
                </label>
              ) : null}
