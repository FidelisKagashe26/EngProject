...
import { GALLERY_ITEMS, SERVICES, STATS } from "./data";
  ...
  <>
    ...
    <section className="relative flex min-h-[88vh] items-center justify-center overflow-hidden bg-[#001534]">
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-4 py-20 text-center sm:px-6 lg:px-8">
        ...
        {/* Stats */}
        <div className="mx-auto mt-16 grid max-w-2xl grid-cols-2 overflow-hidden rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm sm:grid-cols-4">
          {STATS.map((stat, i) => (
            <div
    ...
    <section className="bg-[#faf9fd] py-20 lg:py-28">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {SERVICES.map((s) => (
            <div
              className="group rounded-2xl border border-slate-200 bg-white p-8 shadow-sm transition duration-300 hover:-translate-y-1 hover:border-[#f28c28] hover:shadow-lg hover:shadow-orange-100"
              key={s.title}
            >
              <div className="mb-5 flex h-14 w-14 items-center justify-center rounded-2xl bg-slate-50 text-[#0b2a53] transition group-hover:bg-orange-50 group-hover:text-[#f28c28]">
                <Icon name={s.iconName} size={28} />
              </div>
              <h3 className="mb-3 font-[Manrope,sans-serif] text-lg font-bold text-[#0b2a53]">{s.title}</h3>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {GALLERY_ITEMS.slice(0, 3).map((item) => (
            <div className="group relative h-64 overflow-hidden rounded-2xl sm:h-72" key={item.title}>
              <div
              ...
              <div className="absolute bottom-0 left-0 p-5">
                ...
                <p className="text-xs text-[#adc7f9]">{item.sub}</p>
              </div>
            </div>
          ))}
        </div>
    ...
    {/* ── CTA Banner ── */}
    <section className="bg-[#0b2a53] py-16 lg:py-20">
      <div className="mx-auto max-w-4xl px-4 text-center sm:px-6 lg:px-8">
        ...
        <p className="mx-auto mt-4 max-w-xl text-base text-[#adc7f9]">
        ...
        <div className="mt-8 flex flex-col items-center justify-center gap-4 sm:flex-row">
          ...
          <Link
            className="flex w-full items-center justify-center gap-2 rounded-xl border-2 border-white/40 px-8 py-4 text-sm font-semibold text-white no-underline transition hover:border-white hover:bg-white/10 sm:w-auto"
            to="/about"
          >
